...
// mapped from textual names used in the tests below to actual MFA private
...
// address retrieves the MFA address of a tester account by label, creating
...
func (ap *testerAccountPool) address(account string) common.Address {
	...
	// Resolve and return the MFA address
...
func TestClique(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// batch (https://github.com/MFAChain/mfachain/issues/17593). Whilst this
	...
	for i, tt := range tests {
		...
		for j, block := range blocks {
			// mfachain the header and prepare it for signing
			header := block.Header()
			if j > 0 {
