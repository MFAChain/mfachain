...
import (
	...
	"github.com/MFAChain/mfachain/internal/testlog"
...
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom mfa engine.
	mfa := NewTester([]string{server.URL}, false)
	defer mfa.Close()
	...
	mfa.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := mfa.SealHash(header).Hex(); work[0] != want {
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	// Create the custom mfa engine.
	mfa := NewTester([]string{server.URL}, false)
	mfa.config.Log = testlog.Logger(t, log.LvlWarn)
	defer mfa.Close()
...
func TestStaleSubmission(t *testing.T) {
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			mfa.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, mfa.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch mfa result timeout", id+1)
		}
	}
}
