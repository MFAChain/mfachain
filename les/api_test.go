...
import (
	...
	"github.com/MFAChain/mfachain/p2p/simulations"
	"github.com/MFAChain/mfachain/p2p/simulations/adapters"
...
func getHead(ctx context.Context, t *testing.T, client *rpc.Client) (uint64, common.Hash) {
	...
	if err := client.CallContext(ctx, &res, "mfa_getBlockByNumber", "latest", false); err != nil {
...
func testRequest(ctx context.Context, t *testing.T, client *rpc.Client) bool {
	...
	defer cancel()
	err := client.CallContext(c, &res, "mfa_getBalance", addr, "latest")
	if err != nil {
...
func newLesServerService(ctx *adapters.ServiceContext) (node.Service, error) {
	...
	MFA, err := eth.New(ctx.NodeContext, &config)
	if err != nil {
		return nil, err
	}
	server, err := NewLesServer(MFA, &config)
	if err != nil {
		return nil, err
	}
	MFA.AddLesServer(server)
	return MFA, nil
}
