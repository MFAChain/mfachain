...
func InitDatabaseFromFreezer(db mfadb.Database) {
...
func iterateTransactions(db mfadb.Database, from uint64, to uint64, reverse bool) (chan *blockTxHashes, chan struct{}) {
...
func IndexTransactions(db mfadb.Database, from uint64, to uint64) {
...
func UnindexTransactions(db mfadb.Database, from uint64, to uint64) {
	// short circuit for invalid range
	if from >= to {
		return
	}
