...
// SnapshotTree is an MFA state snapshot tree. It consists of one persistent
...
type Tree struct {
	diskdb mfadb.KeyValueStore      // Persistent database to store the snapshot
...
// background thread.
func New(diskdb mfadb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, async bool) *Tree {
