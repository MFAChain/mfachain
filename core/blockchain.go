...
// Package core implements the MFA consensus protocol.
package core
...
import (
	...
	"github.com/MFAChain/mfachain/trie"
...
type BlockChain struct {
	...
	db     mfadb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default MFA Validator and
// Processor.
func NewBlockChain(db mfadb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool, txLookupLimit *uint64) (*BlockChain, error) {
	if cacheConfig == nil {
	...
	// the head block (mfa cache or clique voting snapshot). Might as well do
	// it in advance.
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	updateFn := func(db mfadb.KeyValueWriter, header *types.Header) {
	...
	delFn := func(db mfadb.KeyValueWriter, hash common.Hash, num uint64) {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			// Migrate all ancient blocks. This can happen if someone upgrades from mfachain
			...
			//
			// But if the `TxIndexTail` is not nil, e.g. mfachain is initialized with
	...
	writeLive := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			// tx indexes)
			if batch.ValueSize() >= mfadb.IdealBatchSize {
				if err := batch.Write(); err != nil {
					return 0, err
				}
				size += batch.ValueSize()
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - mfadb.IdealBatchSize)
			}
...
// sync, mfachain will automatically construct the missing indices and delete
// the extra indices.
func (bc *BlockChain) maintainTxIndex(ancients uint64) {
	// Before starting the actual maintenance, we need to handle a special case,
	// where user might init mfachain with an external ancient database. If so, we
	...
	indexBlocks := func(tail *uint64, head uint64, done chan struct{}) {
		...
		// If the user just upgraded mfachain to a new version which supports transaction
