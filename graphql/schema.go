...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte MFA address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an MFA account at a particular block.
    type Account {
    ...
    # Transaction is an MFA transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an MFA account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an MFA account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an MFA block by number or by hash. If neither is
