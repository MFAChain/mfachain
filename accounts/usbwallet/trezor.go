...
import (
	...
	"github.com/MFAChain/mfachain/accounts/usbwallet/trezor"
...
// or whether the MFA app was not started on it.
...
// MFA address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.MFAAddress)
	if _, err := w.trezorExchange(&trezor.MFAGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.MFATxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.MFATxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
