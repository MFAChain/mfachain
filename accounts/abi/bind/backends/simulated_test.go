...
func TestSimulatedBackend(t *testing.T) {
	...
	if err != mfa.NotFound {
		t.Fatalf("err should be `mfa.NotFound` but received %v", err)
	}
...
func TestSimulatedBackend_EstimateGas(t *testing.T) {
	...
	var cases = []struct {
		...
		{"plain transfer(valid)", mfa.CallMsg{
			From:     addr,
			To:       &addr,
			Gas:      0,
		...
		{"plain transfer(invalid)", mfa.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      0,
			GasPrice: big.NewInt(0),
...
func TestSimulatedBackend_EstimateGasWithPrice(t *testing.T) {
	...
	var cases = []struct {
		...
		{"EstimateWithoutPrice", mfa.CallMsg{
			From:     addr,
			To:       &receipant,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"EstimateWithVeryHighPrice", mfa.CallMsg{
			From:     addr,
			To:       &receipant,
			Gas:      0,
		...
		{"EstimateWithSuperhighPrice", mfa.CallMsg{
			From:     addr,
			To:       &receipant,
			Gas:      0,
...
func TestSimulatedBackend_PendingAndCallContract(t *testing.T) {
	...
	res, err := sim.PendingCallContract(bgCtx, mfa.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	})
	...
	res, err = sim.CallContract(bgCtx, mfa.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	}, nil)
