...
func (n *Node) OpenDatabase(name string, cache, handles int, namespace string) (mfadb.Database, error) {
	if n.config.DataDir == "" {
...
func (n *Node) OpenDatabaseWithFreezer(name string, cache, handles int, freezer, namespace string) (mfadb.Database, error) {
	if n.config.DataDir == "" {
		return rawdb.NewMemoryDatabase(), nil
	}
