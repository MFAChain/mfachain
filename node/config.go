...
import (
	...
	"github.com/MFAChain/mfachain/accounts/external"
	"github.com/MFAChain/mfachain/accounts/keystore"
	"github.com/MFAChain/mfachain/accounts/scwallet"
	"github.com/MFAChain/mfachain/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of mfa is "mfa". If no
	...
	oldMfachainResourceWarning bool
}
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "mfachain", keep that.
	if name == "mfa" || name == "mfa-testnet" {
		name = "mfachain"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "mfa" instances.
var isOldMfachainResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by mfa 1.4 are used if they exist.
	if warn, isOld := isOldMfachainResource[path]; isOld {
		oldpath := ""
		if c.name() == "mfa" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldMfachainResourceWarning, "Using deprecated resource file %s, please move this file to the 'mfa' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "mfachain-keystore")
		ephemeral = keydir
	}
