...
# 1. install mfa for all users
# 2. install optional development tools such as abigen
# 3. create an uninstaller
# 4. configures the Windows firewall for mfa
# 5. create mfa, attach and uninstall start menu entries
# 6. configures the registry that allows Windows to manage the package through its platform tools
# 7. adds the environment system wide variable MFA_SOCKET
...
!define GROUPNAME "MFA"
!define APPNAME "mfachain"
!define DESCRIPTION "Official Go implementation of the MFA protocol"
!addplugindir .\
...
function .onInit
  # make vars are global for all users since mfa is installed global
