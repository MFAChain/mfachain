...
var (
	...
	ethPortFlag = flag.Int("mfaport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the MFA protocol")
	statsFlag   = flag.String("mfastats", "", "Mfastats network monitoring auth string")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for MFA and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // MFA address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an MFA light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // MFA protocol stack
	client *mfaclient.Client   // Client connection to the MFA chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the MFA light client protocol
	...
	// Assemble the mfastats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightMFA
			ctx.Service(&serv)
			return mfastats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := mfaclient.NewClient(api)
...
// close terminates the MFA connection and tears down the faucet.
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the MFA address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/MFAChain/mfachain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and MFA address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the MFA address and profile URL.
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", common.Address{}, errors.New("No MFA address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and MFA address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain MFA address,
