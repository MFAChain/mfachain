...
// Package utils contains internal helper functions for mfachain commands.
package utils
...
import (
	...
	"github.com/MFAChain/mfachain/accounts/keystore"
	"github.com/MFAChain/mfachain/common"
	"github.com/MFAChain/mfachain/common/fdlimit"
	...
	"github.com/MFAChain/mfachain/mfastats"
	"github.com/MFAChain/mfachain/graphql"
	"github.com/MFAChain/mfachain/les"
	"github.com/MFAChain/mfachain/log"
	"github.com/MFAChain/mfachain/metrics"
	"github.com/MFAChain/mfachain/metrics/influxdb"
	...
	"github.com/MFAChain/mfachain/p2p/nat"
	"github.com/MFAChain/mfachain/p2p/netutil"
	"github.com/MFAChain/mfachain/params"
	"github.com/MFAChain/mfachain/rpc"
	whisper "github.com/MFAChain/mfachain/whisper/whisperv6"
...
var (
	...
	EthashCacheDirFlag = DirectoryFlag{
		Name:  "mfaash.cachedir",
		Usage: "Directory to store the mfa verification caches (default = inside the datadir)",
	}
	EthashCachesInMemoryFlag = cli.IntFlag{
		Name:  "mfaash.cachesinmem",
		Usage: "Number of recent mfa caches to keep in memory (16MB each)",
		Value: eth.DefaultConfig.Ethash.CachesInMem,
	}
	EthashCachesOnDiskFlag = cli.IntFlag{
		Name:  "mfaash.cachesondisk",
		Usage: "Number of recent mfa caches to keep on disk (16MB each)",
		Value: eth.DefaultConfig.Ethash.CachesOnDisk,
	}
	EthashCachesLockMmapFlag = cli.BoolFlag{
		Name:  "mfaash.cacheslockmmap",
		Usage: "Lock memory maps of recent mfa caches",
	}
	EthashDatasetDirFlag = DirectoryFlag{
		Name:  "mfaash.dagdir",
		Usage: "Directory to store the mfa mining DAGs",
		Value: DirectoryString(eth.DefaultConfig.Ethash.DatasetDir),
	}
	EthashDatasetsInMemoryFlag = cli.IntFlag{
		Name:  "mfaash.dagsinmem",
		Usage: "Number of recent mfa mining DAGs to keep in memory (1+GB each)",
		Value: eth.DefaultConfig.Ethash.DatasetsInMem,
	}
	EthashDatasetsOnDiskFlag = cli.IntFlag{
		Name:  "mfaash.dagsondisk",
		Usage: "Number of recent mfa mining DAGs to keep on disk (1+GB each)",
		Value: eth.DefaultConfig.Ethash.DatasetsOnDisk,
	}
	EthashDatasetsLockMmapFlag = cli.BoolFlag{
		Name:  "mfaash.dagslockmmap",
		Usage: "Lock memory maps for recent mfa mining DAGs",
	}
	...
	EthStatsURLFlag = cli.StringFlag{
		Name:  "mfastats",
		Usage: "Reporting URL of a mfastats service (nodename:secret@host:port)",
	}
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.GlobalString(DataDirFlag.Name); path != "" {
		if ctx.GlobalBool(LegacyTestnetFlag.Name) || ctx.GlobalBool(RopstenFlag.Name) {
			// Maintain compatibility with older mfachain configurations storing the
...
// for mfachain and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `mfa account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "MFA", ethPeers, "LES", lightPeers, "total", cfg.MaxPeers)
...
// RegisterEthService adds an MFA client to the stack.
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the MFA service: %v", err)
	}
}
...
// RegisterEthStatsService configures the MFA Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		var lesServ *les.LightMFA
		ctx.Service(&lesServ)

		// Let mfastats use whichever is not nil
		return mfastats.New(url, ethServ, lesServ)
	}); err != nil {
		Fatalf("Failed to register the MFA Stats service: %v", err)
	}
}
...
func RegisterGraphQLService(stack *node.Node, endpoint string, cors, vhosts []string, timeouts rpc.HTTPTimeouts) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		return nil, errors.New("no MFA service")
	}); err != nil {
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "mfa.", tagsMap)
		}
	}
}
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node) mfadb.Database {
	var (
...
func MakeChain(ctx *cli.Context, stack *node.Node, readOnly bool) (chain *core.BlockChain, chainDb mfadb.Database) {
	var err error
...
// new format.
//
// e.g. mfa account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// mfa --keystore /tmp/mykeystore --lightkdf account new
//
