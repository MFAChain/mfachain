...
func TestAccountListEmpty(t *testing.T) {
	mfa := runMfachain(t, "account", "list")
	mfa.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	mfa := runMfachain(t, "account", "list", "--datadir", datadir)
	defer mfa.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	mfa := runMfachain(t, "account", "new", "--lightkdf")
	defer mfa.ExpectExit()
	mfa.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	mfa := runMfachain(t, "account", "import", keyfile, "-password", passwordFile)
	defer mfa.ExpectExit()
	mfa.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	mfa := runMfachain(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	mfa := runMfachain(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer mfa.ExpectExit()
	mfa.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(mfa.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(mfa.StderrText(), m) {
...
// https://github.com/MFAChain/mfachain/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	mfa.SetTemplateFunc("keypath", func(file string) string {
