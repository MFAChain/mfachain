...
var (
	...
	retestethCommand = cli.Command{
		...
		Name:        "retesteth",
		Usage:       "Launches mfa in retesteth mode",
		ArgsUsage:   "",
		Flags:       []cli.Flag{rpcPortFlag},
		Category:    "MISCELLANEOUS COMMANDS",
		Description: `Launches mfa in retesteth mode (no database, no network, only retesteth RPC interface)`,
	}
)
...
func (api *RetestethAPI) ClientVersion(ctx context.Context) (string, error) {
	return "mfachain-" + params.VersionWithCommit(gitCommit, gitDate), nil
}
...
func retesteth(ctx *cli.Context) error {
	...
	srv := rpc.NewServer()
	err := node.RegisterApisFromWhitelist(rpcAPI, []string{"test", "mfa", "debug", "web3"}, srv, false)
	if err != nil {
