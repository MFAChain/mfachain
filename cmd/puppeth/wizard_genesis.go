...
func (w *wizard) makeGenesis() {
	...
	switch {
	case choice == "1":
		// In case of mfa, we're pretty much done
...
// importGenesis imports a mfachain genesis spec into puppeth.
...
func (w *wizard) manageGenesis() {
	...
	case "2":
		...
		// Export the native genesis spec used by puppeth and mfachain
		mfachainJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile((mfachainJson), out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", mfachainJson)

		// Export the genesis spec used by Aleth (formerly C++ MFA)
		...
		// Export the genesis spec used by Harmony (formerly MFAJ)
