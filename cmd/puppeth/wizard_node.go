...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.mfastats == "" {
		log.Error("No mfastats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Ethash != nil && !boot {
		fmt.Println()
		if infos.mfaashdir == "" {
			fmt.Printf("Where should the mfa mining DAGs be stored on the remote machine?\n")
			infos.mfaashdir = w.readString()
		} else {
			fmt.Printf("Where should the mfa mining DAGs be stored on the remote machine? (default = %s)\n", infos.mfaashdir)
			infos.mfaashdir = w.readDefaultString(infos.mfaashdir)
		}
	}
	...
	if infos.mfastats == "" {
		...
		infos.mfastats = w.readString() + ":" + w.conf.mfastats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.mfastats)
		infos.mfastats = w.readDefaultString(infos.mfastats) + ":" + w.conf.mfastats
	}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy MFA node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
