...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'mfa --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec mfa --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --mfastats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh
