...
import (
	...
	math2 "github.com/MFAChain/mfachain/common/math"
...
// newAlethGenesisSpec converts a mfachain genesis block into a Aleth-specific
// chain specification format.
func newAlethGenesisSpec(network string, genesis *core.Genesis) (*alethGenesisSpec, error) {
	// Only mfa is currently supported between mfachain and aleth
	...
	spec.Params.BlockReward = (*hexutil.Big)(mfa.FrontierBlockReward)
...
type parityChainSpecPricing struct {
	...
	// Before the https://github.com/paritytech/parity-mfa/pull/11039,
...
// newParityChainSpec converts a mfachain genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only mfa is currently supported between mfachain and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	...
	// mfa has it set from zero
	spec.Params.MaxCodeSizeTransition = 0

	// Disable this one
	spec.Params.EIP98Transition = math.MaxInt64

	spec.Genesis.Seal.MFA.Nonce = types.EncodeNonce(genesis.Nonce)
	spec.Genesis.Seal.MFA.MixHash = (genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
func (spec *parityChainSpec) setByzantium(num *big.Int) {
	spec.Engine.Ethash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(mfa.ByzantiumBlockReward)
...
func (spec *parityChainSpec) setConstantinople(num *big.Int) {
	spec.Engine.Ethash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(mfa.ConstantinopleBlockReward)
...
// pyMFAGenesisSpec represents the genesis specification format used by the
// Python MFA implementation.
type pyMFAGenesisSpec struct {
	Nonce      types.BlockNonce  `json:"nonce"`
	Timestamp  hexutil.Uint64    `json:"timestamp"`
	ExtraData  hexutil.Bytes     `json:"extraData"`
	GasLimit   hexutil.Uint64    `json:"gasLimit"`
...
// newPyMFAGenesisSpec converts a mfachain genesis block into a Parity specific
// chain specification format.
func newPyMFAGenesisSpec(network string, genesis *core.Genesis) (*pyMFAGenesisSpec, error) {
	// Only mfa is currently supported between mfachain and pyMFA
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	spec := &pyMFAGenesisSpec{
