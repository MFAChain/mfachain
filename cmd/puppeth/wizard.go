...
type config struct {
	...
	mfastats  string   // Ethstats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // MFA services known to be running on servers
...
// converts it to an MFA address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by mfastats, which compares IPs textually, not by value.
