...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"mfastats", "explorer", "wallet", "faucet"} {
	...
	// If we have mfastats running, ask whether to make the secret public or not
	if w.conf.mfastats != "" {
		fmt.Println()
		fmt.Println("Include mfastats secret on dashboard (y/n)? (default = yes)")
