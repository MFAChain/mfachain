...
// nodeDockerfile is the Dockerfile required to run an MFA node.
var nodeDockerfile = `
FROM mfa/client-go:latest
...
{{end}}
RUN \
  echo 'mfa --cache 512 init /genesis.json' > mfa.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.mfa/keystore/ && cp /signer.json /root/.mfa/keystore/' >> mfa.sh && \{{end}}
	echo $'exec mfa --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --mfastats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> mfa.sh

ENTRYPOINT ["/bin/sh", "mfa.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an MFA node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.MFA{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.mfa{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
// deployNode deploys a new MFA node container to a remote machine via SSH,
...
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(nodeDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Mfastats":  config.mfastats,
		"Mfaerbase": config.etherbase,
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Mfaashdir":  config.mfaashdir,
		"Network":    network,
		"Port":       config.port,
		"TotalPeers": config.peersTotal,
		"Light":      config.peersLight > 0,
		"LightPeers": config.peersLight,
		"Mfastats":   config.mfastats[:strings.Index(config.mfastats, ":")],
		"Mfaerbase":  config.etherbase,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Mfastats username":        info.mfastats,
	}
	if info.gasTarget > 0 {
		...
		if info.etherbase != "" {
			...
			report["Mfaash directory"] = info.mfaashdir
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 mfa --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.MFA"],
		mfaashdir:  infos.volumes["/root/.mfa"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		mfastats:   infos.envvars["STATS_NAME"],
