...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for mfastats availability")
	if infos, err := checkEthstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["mfastats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["mfastats"] = infos.Report()
		mfastats = infos.config
	}
