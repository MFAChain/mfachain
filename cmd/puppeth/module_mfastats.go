...
// ethstatsDockerfile is the Dockerfile required to build an mfastats backend
// and associated monitoring site.
var ethstatsDockerfile = `
FROM puppeth/mfastats:latest

RUN echo 'module.exports = {trusted: [{{.Trusted}}], banned: [{{.Banned}}], reserved: ["yournode"]};' > lib/utils/config.js
`

// ethstatsComposefile is the docker-compose.yml file required to deploy and
// maintain an mfastats monitoring site.
var ethstatsComposefile = `
version: '2'
services:
  mfastats:
    build: .
    image: {{.Network}}/mfastats
...
// deployEthstats deploys a new mfastats container to a remote machine via SSH,
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	// Build and deploy the mfastats service
	if nocache {
...
// ethstatsInfos is returned from an mfastats status check to allow reporting
...
// checkEthstats does a health-check against an mfastats server to verify whether
// it's running, and if yes, gathering a collection of useful infos about it.
func checkEthstats(client *sshClient, network string) (*ethstatsInfos, error) {
	// Inspect a possible mfastats container on the host
	...
	if err = checkPort(host, port); err != nil {
		log.Warn("Mfastats service seems unreachable", "server", host, "port", port, "err", err)
	}
