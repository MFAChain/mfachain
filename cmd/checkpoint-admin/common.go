...
import (
	...
	"github.com/MFAChain/mfachain/contracts/checkpointoracle"
...
func newClient(ctx *cli.Context) *mfaclient.Client {
	client, err := mfaclient.Dial(ctx.GlobalString(nodeURLFlag.Name))
	if err != nil {
		utils.Fatalf("Failed to connect to MFA node: %v", err)
	}
	return client
}
...
func newContract(client *rpc.Client) (common.Address, *checkpointoracle.CheckpointOracle) {
	...
	contract, err := checkpointoracle.NewCheckpointOracle(addr, mfaclient.NewClient(client))
	if err != nil {
