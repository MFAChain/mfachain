...
// Header represents a block header in the MFA blockchain.
type Header struct {
	header *types.Header
}
...
// Block represents an entire block in the MFA blockchain.
type Block struct {
	block *types.Block
}
...
// Transaction represents a single MFA transaction.
...
// Deprecated: use MFAClient.TransactionSender
