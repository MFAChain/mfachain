...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", mfachain.LightScryptN, mfachain.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.MFA", new NodeConfig());
			node.start();
			...
			MFAClient ec = node.getMFAClient();
	...
	// signatures too. Regression test for mfachain issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "mfa-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/mfa/mfachaintest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for mfachain and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.MFA", "github.com/MFAChain/mfachain/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "mfa.aar"), "mfa.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "MFA", "mfachaintest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
