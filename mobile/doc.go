...
// Package mfa contains the simplified mobile APIs to mfachain.
//
// The scope of this package is *not* to allow writing a custom MFA client
// with pieces plucked from mfachain, rather to allow writing native dapps on
...
// from the mfachain repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original mfachain version.
