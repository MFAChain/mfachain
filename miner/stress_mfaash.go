...
func main() {
	...
	// Pre-generate the mfa mining DAG so we don't race
	mfa.MakeDataset(1, filepath.Join(os.Getenv("HOME"), ".mfa"))
	...
	for _, node := range nodes {
		var MFA *eth.MFA
		if err := node.Service(&MFA); err != nil {
			panic(err)
		}
		if err := MFA.StartMining(1); err != nil {
			panic(err)
		}
	}
	time.Sleep(3 * time.Second)
	...
	for {
		...
		var MFA *eth.MFA
		if err := nodes[index%len(nodes)].Service(&MFA); err != nil {
			panic(err)
		}
		...
		if err := MFA.TxPool().AddLocal(tx); err != nil {
			panic(err)
		}
		nonces[index]++

		// Wait if we're too saturated
		if pend, _ := MFA.TxPool().Stats(); pend > 2048 {
...
func makeMiner(genesis *core.Genesis) (*node.Node, error) {
	// Define the basic configurations for the MFA node
	...
	// Start the node and configure a full MFA node on it
