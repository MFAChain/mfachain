...
import (
	...
	"github.com/MFAChain/mfachain/eth/fetcher"
...
// NewProtocolManager returns a new MFA sub protocol manager. The MFA sub protocol manages peers capable
// with the MFA network.
func NewProtocolManager(config *params.ChainConfig, checkpoint *params.TrustedCheckpoint, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb mfadb.Database, cacheLimit int, whitelist map[uint64]common.Hash) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing MFA peer", "peer", id)

	// Unregister the peer from the downloader and MFA peer set
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("MFA peer connected", "name", p.Name())

	// Execute the MFA handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash(), forkid.NewID(pm.blockchain), pm.forkFilter); err != nil {
		p.Log().Debug("MFA handshake failed", "err", err)
		return err
	}

	// Register the peer locally
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("MFA peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("MFA message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the MFA sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // MFA network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
