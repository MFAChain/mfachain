...
// PeerInfo represents a short summary of the MFA sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // MFA protocol version negotiated
...
func (p *peer) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id,
		fmt.Sprintf("mfa/%2d", p.version),
	)
}

// peerSet represents the collection of active peers currently participating in
// the MFA sub-protocol.
type peerSet struct {
