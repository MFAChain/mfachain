...
import (
	...
	"github.com/MFAChain/mfachain/common/bitutil"
...
func GetChtRoot(db mfadb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func StoreChtRoot(db mfadb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	diskdb, trieTable    mfadb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db mfadb.Database, odr OdrBackend, size, confirms uint64) *core.ChainIndexer {
...
func GetBloomTrieRoot(db mfadb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db mfadb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewBloomTrieIndexer(db mfadb.Database, odr OdrBackend, parentSize, size uint64) *core.ChainIndexer {
