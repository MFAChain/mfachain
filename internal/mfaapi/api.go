...
import (
	...
	"github.com/MFAChain/mfachain/accounts/scwallet"
...
// PublicMFAAPI provides an API to access MFA related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicMFAAPI struct {
	b Backend
}

// NewPublicMFAAPI creates a new MFA protocol API.
func NewPublicMFAAPI(b Backend) *PublicMFAAPI {
	return &PublicMFAAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicMFAAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
	price, err := s.b.SuggestPrice(ctx)
	return (*hexutil.Big)(price), err
}

// ProtocolVersion returns the current MFA protocol version this node supports
func (s *PublicMFAAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicMFAAPI) Syncing() (interface{}, error) {
...
// Sign calculates an MFA ECDSA signature for:
// keccack256("\x19MFA Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/MFAChain/mfachain/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19MFA Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/MFAChain/mfachain/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid MFA signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and mfa.
...
// PublicBlockChainAPI provides an API to access the MFA blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new MFA blockchain API.
...
//
// https://github.com/MFAChain/wiki/wiki/JSON-RPC#eth_sign
...
// PublicDebugAPI is the collection of MFA APIs exposed over the public
// debugging endpoint.
...
func (api *PublicDebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	...
	return fmt.Sprintf("0x%x", mfa.SeedHash(number)), nil
}

// PrivateDebugAPI is the collection of MFA APIs exposed over the private
// debugging endpoint.
...
// Version returns the current MFA protocol version.
